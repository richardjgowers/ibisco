...
SUBROUTINE OUTPUT (I)
  ...
  real(kind=rkind) :: VNBONDP_BEAD, VBONDP_CG, VANGLEP_CG, VTORP_CG, VOOPP_CG
  ...
  !      TOTPOT = VNBOND_ATOM + VBOND + VANGLE + VTOR
  !      WRITE (100,'(4e24.10)') TREAL, EK*CONV, TOTPOT*CONV, EK*CONV+TOTPOT*CONV
  !      WRITE (101,'(5e24.10)') TREAL, VNBOND_ATOM*CONV, VBOND*CONV, VANGLE*CONV, VTOR*CONV
  !      WRITE (110,*) TREAL , TP
  ...
  TotVNBOND = V_NB(2)*CONV + V_NB(1)*CONV
  ...
  WRITE (115, 100)'Tot._Nonbonded_energy:         ', SUM(V_NB)*conv, RVNBONDP
  WRITE (115, 100)'      Nonbonded_Atom_energy:  ', V_NB(1)*conv, RVNBONDP_At
  WRITE (115, 100)'      Nonbonded_Beads_energy: ', V_NB(2)*conv, RVNBONDP_CG
  WRITE (115, 100)'      Nonbonded_mix_energy:   ', V_NB(3)*conv, RVNBONDP_MIX
  WRITE (115, 100)'Tot._Bond_energy:        ', SUM(V_BOND)*conv, RVBONDP
  WRITE (115, 100)'      Bond_Atom_energy:  ', V_BOND(1)*CONV, RVBONDP_At
  WRITE (115, 100)'      Bond_Beads_energy: ', V_BOND(2)*CONV, RVBONDP_CG
  WRITE (115, 100)'      Bond_mix_energy:   ', V_BOND(3)*CONV, RVBONDP_MIX
  WRITE (115, 100)'Tot._Angle_energy:         ', SUM(V_ANGLE)*conv, RVANGLEP
  WRITE (115, 100)'      Angle_Atom_energy:  ', V_ANGLE(1)*CONV, RVAngleP_At
  WRITE (115, 100)'      Angle_Beads_energy: ', V_ANGLE(2)*CONV, RVAngleP_CG
  WRITE (115, 100)'      Angle_mix_energy:   ', V_ANGLE(3)*CONV, RVAngleP_MIX
  WRITE (115, 100)'Tot._Torsion energy:       ', SUM(V_TORSION)*CONV, RVTORP
  WRITE (115, 100)'     Torsion_Atom_energy: ', V_TORSION(1)*CONV, RVTORP_At
  WRITE (115, 100)'     Torsion_Beads_energy:', V_TORSION(2)*CONV, RVTORP_CG
  WRITE (115, 100)'     Torsion_mix_energy:  ', V_TORSION(3)*CONV, RVTORP_MIX

  WRITE (115, 100)'Improper_torsion_energy:  ', SUM(V_OOP)*CONV, RVOOPP
#ifdef DEBUG_OOP
