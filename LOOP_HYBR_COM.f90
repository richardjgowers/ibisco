     ...
     IF (MOD(I, NUPDATE) == 0) THEN
        ...
        ELSE
           ...
           DO IL=1,NVIRTA
              ...
              IF ((VIRTRX(IL)> BOXX2).OR.(VIRTRX(IL) < -BOXX2) &
                   .OR.(VIRTRY(IL)> BOXY2).OR.(VIRTRY(IL)< -BOXY2) &
                   .OR.(VIRTRZ(IL)> BOXZ2).OR.(VIRTRZ(IL)<-BOXZ2)) THEN

                 RPX = VIRTRX(IL)/BOXX2
                 RPY = VIRTRY(IL)/BOXY2
                 RPZ = VIRTRZ(IL)/BOXZ2

                 SPX = RPX - 2.0D0*DBLE(REAL(INT(RPX/2.0D0))) !FIX THIS
                 SPY = RPY - 2.0D0*DBLE(REAL(INT(RPY/2.0D0))) !FIX THIS
                 SPZ = RPZ - 2.0D0*DBLE(REAL(INT(RPZ/2.0D0))) !FIX THIS

                 RPX = (SPX - 2.0D0*DBLE(INT(SPX)))*BOXX2 !FIX THIS
                 RPY = (SPY - 2.0D0*DBLE(INT(SPY)))*BOXY2 !FIX THIS
                 RPZ = (SPZ - 2.0D0*DBLE(INT(SPZ)))*BOXZ2 !FIX THIS
